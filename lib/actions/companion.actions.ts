...
//     pagination
    ...
    if(error) throw new Error(error?.message || 'Companion may be busy')
...
export const addToSessionHistory = async (companionId: string) => {
    ...
    const superbase = createSupabaseClient();
    const {data , error} = await superbase.from('session_history').insert({
...
export const getRecentSessions = async (limit=10 ) => {
    const superbase = createSupabaseClient()
    const {data, error} = await superbase.from('session_history').select(`companions:companion_id (*)`)
...
export const newCompanionPermissions = async () => {
    ...
    const {data, error} = await superbase.from('companions').select('id',{count: 'exact'})
        .eq('author', userId)
